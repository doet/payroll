...
<!--file-->
            ...
            <!--Upload-->            
            <div class="col-sm-7">
                <form action="{{url('AttSave')}}" class="dropzone well" id="dropzone" enctype="multipart/form-data">
                    {!! csrf_field() !!}
                    <input type="hidden" id='datatb' name="datatb" value="uploadfile" />                    
...
////////////////////////////////////file tree
        ...
        if (param != null) {
            ...
            var postData = {datatb:'file',id:param,_token:'{{csrf_token()}}'};                
            $.ajax({
                url: "{{ url('/AttJson/') }}",
                data: postData,
                type: 'POST',
...
/////////////////////////////tabel
    function loadfile(fname){
        var postData = {fname:fname,datatb:'att',_token:'{{csrf_token()}}'};
        $.ajax({
            type: 'POST',
            url: "{{url('AttJson')}}",
            data: postData,
            ...
            success: function(tmp) {
                if (tmp.header){
                    ...
                    $("#priode").html(tmp.priode);

                    if (tmp.nfile)$('#savetodb').prop('disabled', false);

                } else {
                    $("#header").html("<th class='center'> E R R R O R </th>");
                    $("#isinya").html("<tr class='center'><td>E R R O R</td></tr>");                 
